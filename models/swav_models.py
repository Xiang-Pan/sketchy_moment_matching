...
def load(path, map_location="cpu"):
    ...
    if os.path.exists(path):
        ...
        print(f"loading from ai:{path}")
        target_path = path.replace("LABROOTs", "/scratch/xp2030/labs")
        logging.critical(f"loading from ai:{path} to {target_path}")
        os.makedirs(os.path.dirname(target_path), exist_ok=True)
        if "scratch" in path:
            os.system(f"rsync -avzrP ai:{path} {os.path.dirname(target_path)}")
        else:
