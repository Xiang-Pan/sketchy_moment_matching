...
from .leverage_score import compute_leverage
...
def leverage_sampling(data, size, sample_wise_loss=None, use_residual=False):
    if use_residual:
        assert sample_wise_loss is not None, "sample_wise_loss must be provided when use_residual is True"

    leverage_scores = compute_leverage(data, use_residual=use_residual, low_rank=True, n_components=data.shape[1], n_iter=5)
    if use_residual:
        prob = sample_wise_loss * leverage_scores
    ...
    idx_vec = np.random.choice(data.shape[0], size, replace=True, p=prob)

    freq_idx = np.bincount(idx_vec, minlength=data.shape[0])

    weights_vec = freq_idx/(size * prob + 1e-10)
