...
class ResNet_224x224(resnet.ResNet):
    def __init__(self, block, layers, channel: int, num_classes: int, record_embedding: bool = False,
                 no_grad: bool = False, ft_layers="all", **kwargs):
        ...
        print("Freezing layers: ", ft_layers)
        if ft_layers == "-1":
...
def ResNet(arch: str, channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
           pretrained: bool = False, ft_layers: str = "all"):
    arch = arch.lower()
    if pretrained:
        if arch == "resnet18":
            net = ResNet_224x224(resnet.BasicBlock, [2, 2, 2, 2], channel=3, num_classes=1000,
                                 record_embedding=record_embedding, no_grad=no_grad, ft_layers=ft_layers)
            from torchvision.models.resnet import ResNet18_Weights
            ref_net = resnet.resnet18(pretrained=True)
            state_dict = ref_net.state_dict()
        elif arch == "resnet34":
        ...
        # print(url)
        # state_dict = load_state_dict_from_url(, progress=True)
...
def ResNet18(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
             pretrained: bool = False, ft_layers: str = "all"):
    return ResNet("resnet18", channel, num_classes, im_size, record_embedding, no_grad, pretrained, ft_layers)

def LinearResNet18(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
                pretrained: bool = False):
    model = ResNet18(channel, num_classes, im_size, record_embedding, no_grad, pretrained, ft_layers="-1")
    return model

def TwoLayerResNet18(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
                pretrained: bool = False):
    model = ResNet18(channel, num_classes, im_size, record_embedding, no_grad, pretrained, ft_layers="-2")
    return model
