...
import deepcore.datasets as mydatasets
...
def main():
    ...
    parser.add_argument('--test_as_val', default=True, type=str_to_bool, help="if set, test set will be used as validation set")
    ...
    for exp in range(start_exp, args.num_exp):
        if args.save_path != "":
            if args.selection == "Uncertainty":
                method = args.selection + "-" + args.uncertainty
            else:
                method = args.selection
            print(f"dst={args.dataset}, net={args.model}, mtd={method}, exp={start_exp}, epc={args.epochs}, fr={args.fraction}, seed={args.seed}")
            checkpoint_name = "{dst}_{net}_{mtd}_exp{exp}_epoch{epc}_{dat}_{fr}_{seed}".format(dst=args.dataset,
                                                                                            net=args.model,
                                                                                            mtd=method,
                                                                                            dat=datetime.now(),
                                                                                            exp=start_exp,
        ...
        print(mydatasets.__dict__)
        if args.dataset == "cifar10":
            from deepcore.datasets import CIFAR10
            channel, im_size, num_classes, class_names, mean, std, dst_trainval, dst_test = CIFAR10(args.data_path, use_clip="CLIP" in args.model, use_tinynet="TinyNet" in args.model)
        elif args.dataset == "cifar100":
            from deepcore.datasets import CIFAR100
            channel, im_size, num_classes, class_names, mean, std, dst_trainval, dst_test = CIFAR100(args.data_path, use_clip="CLIP" in args.model, use_tinynet="TinyNet" in args.model)
        elif args.dataset == "stanfordcars":
            from deepcore.datasets import StanfordCars
            channel, im_size, num_classes, class_names, mean, std, dst_trainval, dst_test = StanfordCars(args.data_path, use_clip="CLIP" in args.model, use_tinynet="TinyNet" in args.model)
        else:
            raise ValueError("Dataset not supported")
        # channel, im_size, num_classes, class_names, mean, std, dst_trainval, dst_test = mydatasets.__dict__[args.dataset] \
            # (args.data_path, use_clip="CLIP" in args.model, use_tinynet="TinyNet" in args.model)
        args.channel, args.im_size, args.num_classes, args.class_names = channel, im_size, num_classes, class_names
        if not args.test_as_val:
            print(dst_trainval.targets)
            train_indices, val_indices = split_train_val(dst_trainval, 0.01, args.seed)
            dst_train = MySubset(dst_trainval, train_indices)
            dst_val = MySubset(dst_trainval, val_indices)
        else:
            dst_train = dst_trainval
            dst_val = dst_test
            # dst_train = torch.utils.data.Subset(dst_train, train_indices)
            # dst_val = torch.utils.data.Subset(dst_train, val_indices)
        ...
        for model in models:
            ...
            if args.scheduler == "ConstantLR":
                print("------Using ConstantLR(NO) scheduler------")
                #! This is a dummapy scheduler that does nothing.
            ...
            if args.save_path != "" and args.resume == "":
                ...
                save_checkpoint(state={"exp": exp,
                                        "subset": subset,
                                        "total_time": total_time,
                                        "sel_args": selection_args},
                                    path=os.path.join(args.save_path, checkpoint_name + ("" if model == args.model else model+ "_") + "unknown.ckpt"),
                                    epoch=0, 
                                    prec=0.)
            check_fun()
            #! If I set args.epochs=0, I will only perform selection and save the subset.
