...
if __name__ == '__main__':
    ...
    parser.add_argument("--random_prune", type=str2bool, default=False)
    parser.add_argument("--class_conditioned", type=str2bool, default=False)
    ...
    if args.dataset == 'cifar':
        ...
        # images = []
        # ages = []
        # for i in os.listdir('data/utkface_aligned_cropped/crop_part1'):
        #     split = i.split('_')
        #     ages.append(int(split[0]))
        #     images.append('data/utkface_aligned_cropped/crop_part1/' + i)    
        ...
        # # save to csv
        # x_train.to_csv('data/utk_train.csv', index=False)
        # x_test.to_csv('data/utk_test.csv', index=False)
        train_df = pd.read_csv('data/utk_train.csv')
        test_df = pd.read_csv('data/utk_test.csv')
        _, preprocess = clip.load('ViT-B/32', device)
        
        net = LinearCLIP('ViT-B/32', num_classes=1)
        tgt_net = LinearCLIP('ViT-B/32', num_classes=1)
        
        transform_train = preprocess
        transform_test = preprocess
        trainset = UTKDataset(train_df, transform_train)
        testset = UTKDataset(test_df, transform_test)
        wholedataset = UTKDataset(train_df, transform_test)
        pruned_dataset = UTKDataset(train_df, transform_train)
        ...
        transform_train = preprocess
        transform_test = preprocess
        trainset = YearBookDataset(split="train", transform=transform_train)
        testset = YearBookDataset(split="test", transform=transform_test)
        wholedataset = YearBookDataset(split="train", transform=transform_test)
        pruned_dataset = YearBookDataset(split="train", transform=transform_train)
        tgt_optimizer = optim.Adam(tgt_net.parameters(), lr=args.retrain_lr)
        tgt_scheduler = None         
    else:
        raise ValueError(f'Invalid dataset name: {args.dataset}')     
    # breakpoint()
    logging.info(f"Trainset {len(trainset)}")
    logging.info(f"Testset {len(testset)}")   
    ...
    if not args.random_prune:
        ...
        for iter in range(20000):
            ...
            # logging.info(torch.sort(torch.diag(U.t()@new_cov@U)/S))
            # # inspect #
    ...
    if args.random_prune:
        logging.info("Random prune")
        selected_index = torch.topk(torch.randn(len(trainset)) , args.m).indices
    elif args.class_conditioned:
        logging.info("Class conditioned")
        ...
        logging.info("Top m prune")
        selected_index = torch.topk(W , args.m).indices
    ...
    for epoch in range(0, 200):
        train_acc = model_train(epoch, pruned_loader, tgt_net, tgt_optimizer, regression=(args.dataset in ['utk', 'yearbook']))
        test_acc = model_test(epoch, testloader, tgt_net, regression=(args.dataset in ['utk', 'yearbook']))
        logging.info(f"Epoch {epoch}, Train score {train_acc}, Test score {test_acc}")
        if test_acc > best_acc:
            best_acc = test_acc
            logging.info(f"Best model found at epoch {epoch}, score {best_acc}!")
